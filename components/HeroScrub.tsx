...
/** How many viewport-heights of scrolling the whole clip is spread across. */
const SCROLL_SCREENS = 2;

/**
 * The headline holds until TEXT_FADE_START, then fades out by TEXT_FADE_END.
 * It finishes well before the scrub does: the closing shot carries its own
 * on-screen name, and leaving the headline up would have the two overlapping.
 */
 ...
 * it. Rather than let that sit there empty, the real "Get a Free Quote" CTA is
 * positioned exactly on top of it. These are the button's pixel bounds within
 * the 1920x1080 frame, measured off the footage.
 */
const VIDEO_BUTTON = { x: 616, y: 818, w: 652, h: 92 };
...
export default function HeroScrub() {
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      ...
      if (Number.isFinite(duration) && duration > 0) {
        ...
        targetTimeRef.current = Math.min(p * duration, duration - 0.05);
      }
    };
  ...
  const scrimOpacity =
    reduceMotion || progress < TEXT_FADE_END
      ? 1
      : Math.max(0.12, 1 - ((progress - TEXT_FADE_END) / (1 - TEXT_FADE_END)) * 0.88);

  // The aligned CTA appears once the video's own button has animated in.
  const ctaOpacity =
    reduceMotion || progress < CTA_REVEAL_AT
      ? 0
      : Math.min(1, (progress - CTA_REVEAL_AT) / (1 - CTA_REVEAL_AT));
        ...
        {!reduceMotion && ctaRect && (
          <a
            ...
            style={{
              left: ctaRect.left - 2,
              top: ctaRect.top - 2,
              width: ctaRect.width + 4,
              height: ctaRect.height + 4,
