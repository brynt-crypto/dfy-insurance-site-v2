...
export default function HeroScrub() {
   ...
   */
  // Same portrait/landscape split the CSS uses, so the card agrees with the
  // layout around it rather than keying off width.
  const narrow = !!frame && frame.w / frame.h < 4 / 3;

  // The cue fades as soon as scrolling starts — it has done its job by then.
  const cueOpacity = canScrub ? Math.max(0, 1 - progress * 4) : 0;
  const card = frame
    ? {
        logoScale: clamp(0.8, frame.w / 950, 2.9),
        // A phone frame is much taller than it is wide, so the desktop
        // proportions leave a long dead gap between the mark and the button.
        // Pull them toward each other on narrow screens.
        logoTop: frame.h * (narrow ? 0.2 : 0.14),
        ruleW: clamp(220, frame.w * (narrow ? 0.62 : 0.42), 640),
        ctaW: clamp(240, frame.w * (narrow ? 0.78 : 0.36), 520),
        ctaH: clamp(54, frame.h * (narrow ? 0.075 : 0.095), 84),
        ctaTop: frame.h * (narrow ? 0.46 : 0.7),
      }
    : null;
  ...
  return (
    <section
      id="top"
      ref={sectionRef}
      // Scroll length is set in CSS rather than JS so the server and client
      // agree: 8 screens on phones, 15 from md up. 15 screens on a phone is
      // ~12,700px of thumbing before the coverage cards, which is too far.
      // The scrub maths reads the section's real height, so it adapts.
      className={reduceMotion ? "relative" : "dfy-hero-section relative"}
    >
      <div
        ref={frameRef}
        className={`${
          reduceMotion ? "relative" : "sticky"
        } dfy-hero-frame flex min-h-[520px] overflow-hidden bg-[var(--dfy-navy-900)]`}
        style={{
          top: reduceMotion ? undefined : navH,
          height: `calc(100svh - ${navH}px)`,
        }}
      >
        {/* On phones the video gets its own exact 16:9 block at the top of
            the hero, so it is full-width and correctly proportioned with the
            copy underneath it. From md up it goes back to filling the frame
            behind the copy. Letterboxing a 16:9 clip into a portrait frame
            drew it at 27% of the screen height with the text on top — that is
            what made mobile look cluttered and oddly proportioned. */}
        <div
          className="dfy-hero-media"
          style={{ ["--dfy-scrim" as string]: String(scrimOpacity) }}
        >
          <video
            ref={videoRef}
            ...
            aria-hidden="true"
            tabIndex={-1}
            // In the mobile block the box is already 16:9, so cover fits it
            // exactly with no crop. From md up, contain keeps the whole frame
            // visible inside a wider viewport (see .dfy-hero-video for the
            // seam feathering, which is desktop-only).
            className="dfy-hero-video absolute inset-0 h-full w-full"
            style={{ opacity: videoOpacity }}
          >
            {/* Scrubbing needs the whole file buffered, so the 1080p master is
                a 14MB download. Phones get a 720p cut at about half that; it
          ...
          {/* Readability layer: a navy wash plus a stronger gradient on the
              text side, so white copy holds contrast over every frame. Once
              the headline has gone these lift, so the closing card reads at
              full strength rather than through a dark film. On mobile the copy
              is no longer over the video, so only a light wash is needed. */}
          <div
            aria-hidden="true"
            className="dfy-scrim absolute inset-0"
            style={{
              background:
        ...
        <div
          className="dfy-hero-copy dfy-wrap relative w-full"
          style={{
            opacity: textOpacity,
        ...
        >
          <div className="max-w-[46rem]">
            ...
            {/* Dropped on phones so the buttons sit high enough to be seen
                without scrolling. The same copy still appears further down. */}
            <p className="dfy-hero-secondary dfy-measure mt-6 text-[length:var(--dfy-body-l)] text-[var(--dfy-ink-on-navy)]">
              {hero.subhead}
            </p>
            ...
            {/* Portrait cue: arrow above the word, sitting just below the
                buttons so it is inside the reading flow rather than stranded
                at the bottom edge where it was easy to miss. */}
            {!reduceMotion && (
              <div
                aria-hidden="true"
                className="dfy-cue-portrait mt-9 justify-center"
                style={{ opacity: cueOpacity }}
              >
                <span className="flex flex-col items-center gap-1.5 text-[0.75rem] font-semibold uppercase tracking-[0.16em] text-white/75">
                  <svg
                    width="22"
                    height="22"
                    viewBox="0 0 24 24"
            ...
            <ul className="dfy-hero-secondary dfy-hero-secondary-row mt-10 flex-wrap gap-x-7 gap-y-3">
        ...
        {!reduceMotion && (
          <div
            aria-hidden="true"
            className="dfy-cue-landscape absolute inset-x-0 bottom-7 justify-center"
            style={{ opacity: cueOpacity }}
          >
